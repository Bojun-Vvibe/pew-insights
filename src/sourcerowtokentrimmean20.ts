/**
 * source-row-token-trim-mean-20: per-source **20 % symmetrically
 ...
 * with trimming fraction `alpha = 0.20`:
 *
 *     k  = floor(alpha * n)
 *     TM = mean( x_(k+1), x_(k+2), ..., x_(n-k) )
 *
 * That is: drop the bottom `k` and top `k` order statistics
 * (each tail trimmed by 20 % of `n`, rounded down), then take
 * the arithmetic mean of the surviving central `n - 2k` rows.
 * On `n = 100` we drop 20 from each side and average the
 * central 60; on `n = 50` we drop 10 from each side and
 * average the central 30.
 *
 * Place on the L-estimator robustness ladder. Sits ONE rung up
 * from `source-row-token-trim-mean-10` (10 % breakdown,
 * v0.6.204) and ONE rung down from `source-row-token-trim-mean-25`
 * (25 % breakdown, already shipped). Mechanically distinct
 * from `source-row-token-winsorized-mean-20` (same alpha = 0.20
 * but CLIPS the tails to boundary values vs DROPS them):
 *
 ...
 *     TRIM-MEAN-20  (this)     20 % breakdown   DROPS  k tails (k=floor(0.20n))
 *     trim-mean-25             25 % breakdown   DROPS  k tails (k=floor(0.25n))
 *     median                   50 % breakdown
 *
 * The natural distinction from `winsorized-mean-20`: at the
 * same alpha = 0.20, winsorized-mean keeps `n` rows in the
 ...
 * trimmed rows contribute nothing). On a tail-symmetric
 * distribution these are not generally equal whenever the
 * central body mean differs from the boundary midpoint.
 *
 ...
 *   6. Per source: skip if `n < minRows` (default 5 — at
 *      `n = 5` we drop one from each side and average the
 *      inner three, which is the smallest meaningful 20 %
 *      symmetric trimmed mean. At `n < 5`, `k = 0` and TM
 ...
 *      - `--min-rows`       (absolute floor 5)             -> droppedBelowMinRows.
...
export interface SourceRowTokenTrimMean20Options {
  since?: string | null;
  until?: string | null;
  source?: string | null;
  /**
   * Drop sources with fewer than this many rows. Display filter.
   * Must be an integer >= 5 (need k = floor(0.20 n) >= 1 to
   * actually trim — at n < 5, k = 0 and TM degenerates to
   * the arithmetic mean). Default 5.
   */
  minRows?: number;
  /**
...
export interface SourceRowTokenTrimMean20Row {
  source: string;
  rowsKept: number;
  /** k = floor(0.20 * rowsKept). Each tail discards exactly k order statistics. */
  ...
  mean: number;
  /** 20 % symmetrically trimmed mean — central (rowsKept - 2k) rows. */
  trimMean: number;
  /** Signed gap trim_mean - mean. */
  tmMeanGap: number;
}

export interface SourceRowTokenTrimMean20Report {
  generatedAt: string;
  ...
  sources: SourceRowTokenTrimMean20Row[];
}

const ABSOLUTE_MIN_ROWS = 5;
...
export function buildSourceRowTokenTrimMean20(
  queue: QueueLine[],
  opts: SourceRowTokenTrimMean20Options = {},
  ...
  let totalRowsKept = 0;
  const allRows: SourceRowTokenTrimMean20Row[] = [];
  ...
  for (const [source, samples] of perSource.entries()) {
    ...
    // Defensive: by construction with n >= minRows >= 5 and TRIM_FRACTION = 0.20,
    // k >= 1 and n - 2k >= 3, so the central window is always non-empty.
    let totalSum = 0;
  ...
  const survived: SourceRowTokenTrimMean20Row[] = [];

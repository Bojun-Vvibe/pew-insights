/**
 * source-row-token-winsorized-mean-20: per-source **20 %
 ...
 * with winsorization fraction `alpha = 0.20`:
 *
 *     k  = floor(alpha * n)
 ...
 * 20 % of each tail in to the boundary of the central 60 % —
 ...
 *   - **Breakdown 20 %**: a single arbitrarily-large row that
 ...
 *     `floor(0.20 * n)` rows on one side. This matches the
 *     breakdown of the 20 % trimmed mean, and is **strictly
 ...
 *     division by zero possible. (At `n < 5`, `k = 0` and
 *     WM degenerates to the arithmetic mean, so we require
 *     n >= 10 by default to ensure k = 2 and meaningful
 ...
 *     dominated by extremes; WM-20 *clips* the 20 % tails to
 ...
 *     to denominator); WM-20 *replaces* the bottom 20 % and
 *     top 20 % with boundary values (they contribute the
 *     boundary value to numerator AND one row to denominator).
 *     Different fraction (20 vs 25), different mechanism
 *     (clip vs drop), different breakdown (20 % vs 25 %).
 *   - **Distinct from the median**: median weights only the
 *     central `1` (or `2`) order statistics; WM-20 weights
 *     every row.
 *   - **Distinct from `source-row-token-midhinge`**:
 *     midhinge = (q1 + q3) / 2 uses *zero* central rows
 *     (just two boundary quantiles); WM-20 uses *all* rows.
 *   - **Distinct from `source-row-token-mid-range`**:
 *     mid-range = (min + max) / 2 listens to *only* the two
 *     extreme tails (0 % breakdown); WM-20 *clips* those
 *     tails (20 % breakdown). They are opposites on the
 *     L-estimator robustness spectrum.
 *   - **Distinct from `source-row-token-trimean`**:
 *     trimean = (q1 + 2*median + q3) / 4 weights three
 *     quantiles; WM-20 weights every row equally after
 *     clipping. Trimean cares about three points; WM-20
 ...
 *     scale-invariant *shape* statistics; WM-20 is location
 *     in token units.
 ...
 *     translation-equivariant; WM-20 is a linear L-estimator,
 ...
 *      `n = 10` we replace two from each side, the smallest
 *      meaningful symmetric 20 % winsorized mean. At `n < 5`,
 *      `k = 0` and WM degenerates to the arithmetic mean).
 *   7. Sort ascending; compute `k = floor(0.20 * n)`,
 ...
 *   - n exactly 10: k = 2, clipped tails are `x_(1), x_(2)` -> `x_(3)`
 *     and `x_(10), x_(9)` -> `x_(8)`; central window x_(3)..x_(8)
 *     enters at original values.
 *   - n exactly 20: k = 4, clipped tails are x_(1)..x_(4) -> x_(5)
 *     and x_(20)..x_(17) -> x_(16); central window x_(5)..x_(16)
 *     enters at original values.
 *
...
export interface SourceRowTokenWinsorizedMean20Options {
   ...
   * Must be an integer >= 10 (with alpha = 0.20, k = floor(0.20 n)
   * — at n = 10, k = 2, the smallest meaningful symmetric 20 %
   * winsorization). Default 10.
   */
  minRows?: number;
  /**
...
export interface SourceRowTokenWinsorizedMean20Row {
  source: string;
  rowsKept: number;
  /** k = floor(0.20 * rowsKept). Each tail clips exactly k order statistics. */
  ...
  mean: number;
  /** 20 % symmetrically winsorized mean — clip both tails to boundaries, then arithmetic mean. */
...
export interface SourceRowTokenWinsorizedMean20Report {
  generatedAt: string;
  ...
  sources: SourceRowTokenWinsorizedMean20Row[];
}

const ABSOLUTE_MIN_ROWS = 10;
const WINSORIZE_FRACTION = 0.2;
...
export function buildSourceRowTokenWinsorizedMean20(
  queue: QueueLine[],
  opts: SourceRowTokenWinsorizedMean20Options = {},
  ...
  let totalRowsKept = 0;
  const allRows: SourceRowTokenWinsorizedMean20Row[] = [];
  ...
  for (const [source, samples] of perSource.entries()) {
    ...
    // WINSORIZE_FRACTION = 0.20, k >= 2 and n - 2k >= 6, so the
  ...
  const survived: SourceRowTokenWinsorizedMean20Row[] = [];

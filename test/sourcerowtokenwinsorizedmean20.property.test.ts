/**
 * Property tests for source-row-token-winsorized-mean-20
...
test('winsor-property: WM == mean iff series is "tail-symmetric around clip-replacement"', () => {
  ...
  // sorted same; k=2; lo=sorted[2]=7; hi=sorted[7]=7;
  // sumWinsor = 2*7 + (7+7+7+7+7+7) + 2*7 = 14+42+14 = 70; wm = 7.0
...
test('winsor-property: trim-mean-25 sits closer to median than WM-20 on right-skew', () => {
  // For a right-skewed series, both WM-20 and trim-mean-25 pull the
  // location estimate toward the body, but trim-mean-25 (k = floor(0.25 n))
  // discards more rows from each tail than WM-20 (k = floor(0.20 n))
  ...
  // trim-mean-25 sits at or below winsorized-mean-20 on right-skew
...
test('winsor-property: appending a row INSIDE the central window does not change wmMeanGap sign', () => {
  ...
  // large enough that floor(0.20 (n+1)) == floor(0.20 n)), and the
  ...
  const before = buildSourceRowTokenWinsorizedMean20(mkSeries('s1', base), {
    generatedAt: GEN,
  }).sources[0]!;
  // Insert a body-region value (10) — well inside [lo=5, hi=16]
  const after = buildSourceRowTokenWinsorizedMean20(
    mkSeries('s1', [...base, 10]),
    { generatedAt: GEN },
  ).sources[0]!;
...
test('winsor-property: WM equals mean when k = 0 (boundary-condition robustness)', () => {
  ...
  // at k = 0. Use exactly n = 10 with a flat series — k = 2, but
...
test('winsor-property: clippedPerTail == floor(0.20 * rowsKept) holds across many sizes', () => {
  for (const n of [10, 11, 13, 19, 20, 21, 49, 50, 99, 100, 137, 256, 500]) {
    ...
    assert.equal(
      r.clippedPerTail,
      Math.floor(0.2 * n),
      `n=${n}: clippedPerTail should be floor(0.20 * ${n})`,
    );

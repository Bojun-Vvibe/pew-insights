...
// ---------- refinement: full integer Lehmer ladder L_8 .. L_11 monotonicity ----------
//
// End-to-end Lehmer-monotonicity pin spanning the ladder L_8 (v0.6.197)
// through L_11 (v0.6.200). For every adjacent rung we assert
// L_k <= L_{k+1} (numerical-tolerance guarded). Strictly stronger
// than the per-builder pairwise checks because it exercises FOUR
// builders against a SINGLE shared input series and verifies every
// adjacent step *and* the transitive chain end-to-end.
test('ladder property: L_8 <= L_9 <= L_10 <= L_11 holds at every adjacent step over 80 random integer series', () => {
  const r = rng(0xb1c2d3e4);
  let trialsRun = 0;
  for (let trial = 0; trial < 80; trial += 1) {
    ...
    // Final transitive chain — strictly weaker than the three pairwise
    // checks above but pinned for documentation purposes.
    assert.ok(
      L8 <= L11 + eps,
      `L_8 <= L_11 (transitive) expected: L_8=${L8}, L_11=${L11}, xs=${JSON.stringify(xs)}`,
    );
    trialsRun += 1;
  }
  assert.ok(
...
test('ladder property: constant positive series collapses L_8 = L_9 = L_10 = L_11 = c', () => {
